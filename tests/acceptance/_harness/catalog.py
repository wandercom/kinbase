    ...
    clauses(
        ...
        every("cells", "each declared cell is executed with a real native transition",
              ...
              is_true("states_match",
                      "C15: public observations, linked facts and Unknowns satisfy the frozen expectations"),
              ...
              is_true("adapter_receipts_present",
...
_o(
    ...
    clauses(
        covers("skew_dispositions", ("CLOCK_SKEW",),
               "receipt-time claims outside the five-minute bound quarantine as CLOCK_SKEW (R-14)"),
        ...
        every("cursors_exercised_detail", "each cursor sees both skew polarities",
              ...
              is_true("positive_skew_quarantined", "future receipt-time claim quarantined"),
              is_true("negative_skew_quarantined", "past receipt-time claim quarantined"),
...
_o(
    ...
    clauses(
        ...
        is_true("harness_recomputation_agrees",
                "macro-F1 equals the instrument mean of per-label F1 (R-13)"),
    ),
...
_o(
    ...
    clauses(
        every("receipts", "each destination returns its own saga receipt",
              ...
              min_len=2),
        equals("destinations", ["codebase", "company"], "both shared destination receipts exist"),
        is_true("independent_receipts", "every destination has its own receipt identity"),
    ),
...
_o(
    ...
    clauses(
        ...
        is_true("all_committed", "both admissions commit"),
        equals("admitted_canary_occurrences", 0, "the canary is absent from admitted events and receipts"),
        is_true("private_taint_present", "the private source record records taint"),
        is_true("deidentify_retains_taint", "checkpoint/replay preserves the private taint"),
    ),
    surfaces=("private admission audit ledgers",),
    vectors=("private marker plus architectural fact",),
...
_o(
    ...
    clauses(
        ...
        equals("doctor_refusal_code", "HOOK_APPROVAL_REQUIRED",
               "C9: doctor reports missing planned host entries"),
        equals("install_exit_code", 0, "C9: install writes the user-level plan"),
        nonempty("installed_files", "installation changes the planned files"),
        is_true("planned_paths_relative_to_home", "C16: file paths are HOME-relative"),
        is_true("installed_paths_match_plan", "only and all planned paths change"),
        is_true("installed_plan_digest_matches", "installation returns the plan digest"),
        is_true("installed_contents_match_plan", "C9: installed bytes equal the displayed diff"),
        equals("project_files_changed", 0, "C9: host project settings stay untouched"),
...
_o(
    ...
    clauses(
        ...
        is_true("projections_match", "both host projections read the same admitted corpus"),
    ),
    surfaces=("kinbase session observe --json",),
