...
- **Typed errors and exit codes.** Every failure is a `ContractError` whose code is in `error.rs::CODES` with the exit that code implies (`ContractError::new` turns a mismatch into `RUN_INTEGRITY_FAILED`). Hook dispatch also puts the reason on stderr in host mode — the host hides stdout.
