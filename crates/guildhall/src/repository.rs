            ...
            for reference in &fact.company_refs {
                ...
                match (company_fact.as_ref(), live_digest.as_deref()) {
                    ...
                    (Some(_), Some(digest)) if digest == reference.semantic_digest => {
                        ...
                        record["digest_attribution"] = json!({"owner_role": "none", "owner_matches_expected": true});
                        record["reference_resolved"] = Value::Bool(true);
                        let expected_projection = freshness
                            .map(|freshness| freshness.projection(stricter == "safety_critical", self.trust.certificate_valid).0)
                            .unwrap_or("trusted");
