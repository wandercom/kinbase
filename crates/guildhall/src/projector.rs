...
pub fn project(
    ...
    candidates.sort_by(|left, right| {
        right
            .distortion
            .loss_if_absent
            .cmp(&left.distortion.loss_if_absent)
    ...
    for fact in candidates {
        ...
        let cost = 200;
        let marginal = i64::from(fact.distortion.loss_if_absent)
            .saturating_mul(i64::from(relevance))
            .saturating_div(10_000)
            + i64::from(authority_gain)
            + i64::from(complementarity)
            - i64::from(redundancy)
            - i64::from(cost);
        if marginal <= 0 {
            omitted += 1;
