...
impl Client {
    pub fn new(url: &str, token: TokenRecord, key: PrivateKey, root: Option<PublicKey>, cache_root: PathBuf) -> Result<Self, ContractError> {
    ...
    fn nonce(&self) -> String {
        ...
        format!("{}{:03}", crate::time::now_utc().timestamp_millis(), sequence % 1000)
    }

    pub fn request(&self, method: &str, path: &str, body: Option<&Value>) -> Result<Response, ContractError> {
        let full_path = format!("{}{}", self.base_path, path);
        let body_bytes = body.map(crate::json::canonical_bytes).unwrap_or_default();
        let nonce = self.nonce();
        let expires_at = crate::time::plus_seconds(&crate::time::now_rfc3339_millis(), 60).map_err(ContractError::internal)?;
        let signed = json!({
            "method": method,
            "path": full_path,
        ...
        let signature = self.key.sign("receipt", &crate::json::canonical_bytes(&signed))?;
        let headers = vec![
            ("Authorization".to_owned(), format!("Bearer {}", self.token.token)),
            ("X-Guildhall-Nonce".to_owned(), nonce),
            ("X-Guildhall-Expires-At".to_owned(), expires_at),
            ("X-Guildhall-Client-Key".to_owned(), self.key.public().to_hex()),
            ("X-Guildhall-Signature".to_owned(), signature),
        ];
        self.bytes_sent.set(self.bytes_sent.get() + body_bytes.len() as u64 + 256);
        http::request(&self.host, self.port, method, &full_path, &headers, &body_bytes, self.read_timeout)
    }
    ...
    pub fn snapshot(&self) -> Result<Value, ContractError> {
        ...
        let root = self
            .root
            .as_ref()
            .ok_or_else(|| ContractError::invariant("a Company root public key is required to verify snapshots"))?;
        let signer = PublicKey::verify_document("receipt", &snapshot).ok_or_else(|| {
            ContractError::integrity("SIGNATURE_INVALID", "Company snapshot signature failed", "Quarantine the snapshot and contact the Company steward.")
        })?;
        if signer != *root {
            return Err(ContractError::integrity("SIGNATURE_INVALID", "Company snapshot is not signed by the configured root key", "Verify the root public key in the user config."));
        }
        if crate::json::get_str(&snapshot, "client_nonce") != Some(nonce.as_str()) {
            return Err(ContractError::integrity("SIGNATURE_INVALID", "Company snapshot does not echo the fresh client nonce (replay)", "Refresh again; a replayed snapshot cannot rebuild trust."));
        }
        Ok(snapshot)
    }
    ...
    pub fn certificates_for_hint(&self, hint: &str) -> Result<Vec<Value>, ContractError> {
        ...
        Ok(crate::json::get_array(&body, "certificates").cloned().unwrap_or_default())
    }

    pub fn fact_digest(&self, fact_id: &str, version: &str, alg: &str) -> Result<Value, ContractError> {
        self.get_ok(&format!("/facts/{fact_id}/digest?version={version}&alg={alg}"))
    }
...
pub fn deliver_to_channel(channel: &str, body: &Value, key: &PrivateKey) -> Result<Response, ContractError> {
    ...
    http::request(&host, port, "POST", &path, &headers, &bytes, Duration::from_secs(5))
}
