...
pub fn explain(
    ...
    let key_events: Vec<crate::model::FactEvent> = data.events.iter().map(|event| event.event.clone()).collect();
    ...
    let view = reduce(data, &store_name_value, as_of, None, Some(authority_snapshot_cursor.clone()))?;
    ...
    if has_codebase {
        if let Ok(repository) = Repository::discover(repo) {
            ...
            if let Ok(context) = crate::repository::RepoContext::load(crate::launcher::Launcher::load()?, repo, true) {
                if let Ok((resolved_view, _counts, company_references)) = context.current_view(&as_of.as_of, Some(authority_snapshot_cursor.as_str())) {
                    resolved_current = resolved_view.facts.iter().find(|fact| fact.logical_key == logical_key).cloned();
    ...
    let current = resolved_current.as_ref().or_else(|| view.facts.iter().find(|fact| fact.logical_key == logical_key));
    ...
    let operational_statement = key_events
        ...
        .find(|event| {
            event.authority_scope.starts_with("environment:")
                && matches!(event.atom_kind.as_str(), "observation" | "runtime_trace" | "test_runtime_evidence")
        })
    ...
    let architecture_rewritten = key_events.iter().any(|event| {
        event.statement.to_ascii_lowercase().contains("architecture")
            && event.statement.to_ascii_lowercase().contains("rewritten: true")
    });
    let authorized_parent_bound_event_resolves = key_events.iter().any(|event| {
        event.parents.len() >= 2
            && event
                .parents
                .iter()
                .all(|parent| key_events.iter().any(|candidate| candidate.event_id == *parent))
            && trace.is_some_and(|trace| trace.state == "current" && trace.conflict_event_ids.is_empty())
    });
...
fn last_numeric_token(statement: &str) -> Option<String> {
    statement
        .split(|character: char| character.is_ascii_whitespace() || character.is_ascii_punctuation())
        .rev()
        .find(|token| !token.is_empty() && token.chars().all(|character| character.is_ascii_digit()))
        .map(str::to_owned)
}
