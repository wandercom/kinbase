# Guildhall Factory Tester remediation continuation 004

Resume the same implementation-blind Claude Tester identity
`7b88a154-cc04-48b3-854f-08902f264c66` from its clean terminal commit
`263d41d77ae11e4e62f2ce2716378a8e1ebdc7f2`. A fresh, distinct,
implementation-blind Detector Reviewer has now blocked that exact artifact on
22 findings. Its exact report follows below and is the sole new review input.

Close all 22 findings as acceptance-instrument defects. Do not optimize for a
green wrapper: each gate must causally observe the native product behavior,
prerequisite state, and per-node mutation it claims. Do not relax the ratified
specification, thresholds, execution census, fail-closed categories, or oracle
boundary.

The existing Factory restrictions remain exact:

- Modify only `tests/**` and Tester-owned fixtures below it.
- Read only this repository's ratified `spec/**`, your own `tests/**`, and
  the report delivered on stdin.
- Do not read or execute product code. Do not inspect Coder, Validator,
  Detector Reviewer, main repository, run-control, Kindex, memory, or another
  agent's state.
- Do not delegate, use the web, contact another role, or obtain hidden
  implementation information.
- Run only collection and product-independent acceptance-instrument checks.
- Replace post-observation interposers with causal, independently witnessed
  mutation mechanisms. Kill accounting must establish every named node fails
  for the intended reason, not merely that a batch returned nonzero.
- Couple every obligation to its actual checker and make native prerequisites
  non-optional. Missing fixture, rights, environment, collection, timeout,
  certificate, registry, service, or gold state is `INVALID_HARNESS`, never a
  product failure or a pass.
- Keep all product-bound selectors opaque and verify the complete product input
  dataflow, including argv, cwd, stdin, environment, service payloads, paths,
  session IDs, and committed history.
- Do not fabricate a human rightsholder grant or a completed Reviewer-owned
  auxiliary selection. Make the pool, provenance digest, deterministic
  generation, selection protocol, and exact candidate bytes ready. If a named
  human ratification is genuinely indispensable, complete and commit every
  independently possible repair first, then emit one concrete
  `FACTORY_QUESTION` naming the exact bytes and authority needed.
- Inspect the entire tests-only delta, commit it on
  `factory/tester-ac8a13d1`, and leave the worktree clean.

The original USD 75 Tester remediation authorization remains the total cap.
Verified combined spend is USD 48.64545275. This continuation's launcher hard
ceiling is USD 26.35; do not exceed it or infer more authority from the user's
separate GLM provider buffer.

End exactly with one of:

`FACTORY_STATUS: DONE <full commit SHA>`
`FACTORY_STATUS: BLOCKED <reason>`
`FACTORY_QUESTION: <one concrete authority question>`

## Exact fresh Detector Reviewer report
